...
using namespace std;
	...
	//pass all neighbours of the given element to the algorithm using addNeighbourToOpenList(string name, int thisCost, int guess)
	void testmap::analyzeElement(string element, string end)
	{
		if(element =="1|1")
		{
			addNeighbourToOpenList("1|3",4,getGuess("1|3",end));
			addNeighbourToOpenList("2|2",2,getGuess("2|2",end));
	...
	int testmap::getGuess(string start, string end)
	{
		...
		return ceil(sqrt(pow((double)((start.at(0) - 48)-(end.at(0) - 48)),2)+pow((double)((start.at(2) - 48)-(end.at(2) - 48)),2)));
	}
